...
namespace Infosys\EPCconnect\Model\Import;
...
class Product extends \Magento\CatalogImportExport\Model\Import\Product
{
    ...
    protected function _deleteProducts()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            if ($idsToDelete) {
                ...
                try {
                    ...
                    $this->deleteVehicleProductMapping($idsToDelete);
                } catch (\Exception $e) {
        ...
        $this->vehicleLogger->info("Products Deleted : " . $this->countItemsDeleted);
        return $this;
    }

    /**
     * Delete Vehicle Product Mapping
     *
     * @param array $productIds
     * @return void
     */
    protected function deleteVehicleProductMapping($productIds)
    {
        try {
            $this->countItemsDeleted += $this->_connection->delete(
                $this->_connection->getTableName(VehicleProductMappingInterface::VEHICLE_PRODUCT_MAPPING_TABLE),
                $this->_connection->quoteInto(VehicleProductMappingInterface::PRODUCT_ID . ' IN (?)', $productIds)
            );
    ...
    protected function _saveProductsData()
    {
        ...
        $this->_coreSession->setData('updateOptions', []);
    ...
    protected function _prepareRowForDb(array $rowData)
    {
        ...
        $rowData = $this->customColValues($rowData);
    ...
    /**
     * Get existing data for attributes
     *
     * @param array $bunch
     * @param array $attribute
     * @return array
     */
    protected function getExistingAttributesData($bunch, $attribute)
    {
        //create array for all skus
        $skus = array_map(
            'strval',
        ...
        //attribute to select
        $productCollection = $productCollection->addAttributeToSelect($attribute, ['notnull' => true]);
        $productCollection = $productCollection->addFieldToFilter('sku', ['in' => $skus]);
        $attributesData = $productCollection->getData();

        $finalData = [];
        if (!empty($attributesData)) {
            array_walk(
                $attributesData,
                function (&$val, $key) use (&$finalData, &$attribute) {
                    $sku = $val['sku'];
                    $finalData[$sku][$attribute] = $val[$attribute];
                }
            );
        }
    ...
    protected function _saveProducts()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            //fetch value for `hazmat_flag` and `do_not_ship` attriubutes to match with csv data
            $oldHazmatData = $this->getExistingAttributesData($bunch, 'hazmat_flag');
            $oldDNSData = $this->getExistingAttributesData($bunch, 'do_not_ship');
            $productMediaData = $this->getExistingProductMediaGallery($bunch);            
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                //Changes for Do not ship
                $existingHazmatFlag = isset($oldHazmatData[$rowSku]['hazmat_flag']) ? $oldHazmatData[$rowSku]['hazmat_flag'] : '';
                $existingDoNoShip = isset($oldDNSData[$rowSku]['do_not_ship']) ? $oldDNSData[$rowSku]['do_not_ship'] : '';
                
                $hazmatFlag = isset($rowData['hazmat_flag']) && !empty($rowData['hazmat_flag']) ? $rowData['hazmat_flag'] : $existingHazmatFlag;
                $doNotShip = isset($rowData['do_not_ship']) && !empty($rowData['do_not_ship']) ? $rowData['do_not_ship'] : $existingDoNoShip;
            
                if ($hazmatFlag == 'Y' || $doNotShip== 'Y') {
                    $rowData['shipperhq_shipping_group'] = 'Do Not Ship';
                } elseif (($hazmatFlag == $this->getEmptyAttributeValueConstant() || $hazmatFlag == 'N')
                    || ($doNotShip == $this->getEmptyAttributeValueConstant() || $doNotShip == 'N')) {
                    $rowData['shipperhq_shipping_group'] = ' ';
                }
                ...
                // 4.2. Vehicle products mapping phase
                if (isset($rowData['model_year_code'])) {
                    $mappingVehicleProducts[] = ['sku' => $rowData['sku'], 'model_year_code' => $rowData['model_year_code']];
                }
                ...
                foreach (array_keys($imageHiddenStates) as $image) {
                    ...
                    if (
                        empty($rowImages[self::COL_MEDIA_IMAGE])
                ...
                foreach ($rowImages as $column => $columnImages) {
                    if(array_key_exists($rowSku, $productMediaData) && !in_array($columnImages[0], $productMediaData[$rowSku]) ){                    
                        foreach ($columnImages as $columnImageKey => $columnImage) {
                            $uploadedFile =  $columnImage;

                            if ($column === self::COL_MEDIA_IMAGE) {
                                $rowData[$column][] = $uploadedFile;
                            }
                            
                            if ($uploadedFile != $this->getEmptyAttributeValueConstant()) {                           
            ...
            $this->saveProductEntity($entityRowsIn, $entityRowsUp)
                ...
                ->_saveVehicleProductMapping($mappingVehicleProducts)
        ...
        $this->vehicleLogger->info("Products Created : " . $this->countItemsCreated . ", Updated : " . $this->countItemsUpdated);
    ...
    /**
     * Scheduled import doesn't have this as an input so we have to force the default as NULL
     *
     * @return void
     */
    ...
    /**
     * Save vehicle product mapping
     *
     * @param array $data
     * @return $this
     */
    protected function _saveVehicleProductMapping(array $data)
    {
        $mappingImport = $this->vehicleConfig->enableVehicleProductMappingImport();
        $fitmentImport = $this->vehicleConfig->enableVehicleFitmentImport();
        ...
        if ($mappingImport) {
            $this->vehicleLogger->info("Vehicle product mapping import is enabled");
            if ($data) {
            ...
            if ($entityList) {
                ...
                if ($rows) {
                    $this->_connection->insertOnDuplicate(VehicleProductMappingInterface::VEHICLE_PRODUCT_MAPPING_TABLE, $rows, $validColumnNames);
                }
            }
        }
        //add fitment queue
        if ($productIdLists && $fitmentImport) {
            $this->vehicleLogger->info("Vehicle fitment import is enabled");
            $this->_connection->insertOnDuplicate('vehicle_fits_queue', $productIdLists);
        }
    ...
    private function getProductId($sku)
    {
        ...
        $select = $this->_connection->select()->from(
            ...
            $this->_connection->quoteInto('sku IN (?)', $sku)
        );
        $roduct = $this->_connection->fetchOne($select);
        return $roduct;
    }
    ...
    private function getVehicleIds($modeYearCodes, $rowNum)
    {
        ...
        $tableName = $this->_connection->getTableName(VehicleInterface::VEHICLE_TABLE);
        $select = $this->_connection->select()->from(
            $tableName
        )->where("(model_code, model_year) IN " . $modelYearCodeValues);
        $vehicleIds = $this->_connection->fetchCol($select);
        return $vehicleIds;
    }
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if ($this->skuProcessor->getNewSku($sku) !== null) {
                    ...
                    if (
                        !empty(array_diff_assoc($row, $existingStockItemData))
    ...
    public function validateRow(array $rowData, $rowNum)
    {
        ...
        if ($this->isNeedToValidateUrlKey($rowData)) {
            ...
            foreach ($storeCodes as $storeCode) {
                ...
                if (
                    empty($this->urlKeys[$storeId][$urlPath])
        ...
        if (
            !empty($rowData['new_from_date']) && !empty($rowData['new_to_date'])
        ) {
    ...
    {
        if (
            !empty($rowData[self::COL_SKU]) && empty($rowData[self::URL_KEY])
            && ($this->getBehavior() === Import::BEHAVIOR_APPEND || $this->getBehavior() === Import::BEHAVIOR_REPLACE)
     ...
     */
    private function customColNames()
    {
        $colnames = ['product_websites', 'is_in_stock'];
        return $colnames;
    }

    /**
     * Required missing columns data
     *
     * @param array $rowData
     */
    private function customColValues($rowData)
    {
        $separator = $this->getMultipleValueSeparator();

        if (!isset($rowData['website']) || (isset($rowData['website']) && $rowData['website'] == $this->getEmptyAttributeValueConstant())) {
            //assign products to all websites
            $websites = $this->_allWebsites;            
            $sites = implode($separator, $websites);
            $rowData['product_websites'] = $sites;
            $rowData['product_website_ids'] = implode(",", array_keys($websites));            
        } else {
            $rowData['product_websites'] = $rowData['website'];
            $rowData['product_website_ids'] = implode(",", $this->getWebsiteIdsByCode($rowData['website']));            
        }

        //check if product is saleable
        if (isset($rowData['saleable']) && !empty($rowData['saleable'])) {
            $saleable = $rowData['saleable'];
            if ($saleable == 'Y') {
                $rowData['is_in_stock'] = true;
                $rowData['manage_stock'] = 0;
            } else if ($saleable == $this->getEmptyAttributeValueConstant() ||  $saleable == 'N') {
                $rowData['is_in_stock'] = false;
                $rowData['manage_stock'] = 1;
            }
        }

        if (isset($rowData['category'])) {
            $rowData['categories'] = $rowData['category'];
        }

        //assign PLP page image
        if (!isset($rowData['small_image']) && isset($rowData['base_image'])) {
            $rowData['small_image'] = $rowData['base_image'];
        }

        if (!isset($rowData['swatch_image']) && isset($rowData['base_image'])) {
            $rowData['swatch_image'] = $rowData['base_image'];
        }

        if (!isset($rowData['thumbnail_image']) && isset($rowData['base_image'])) {
            $rowData['thumbnail_image'] = $rowData['base_image'];
        }

        //TODO handle edge case where only one of price or product_online is passed in.

        //Default product_online to 1
        $productOnline = 1;
        //If not below threshold, set based on product_online,
        if (isset($rowData['product_online'])) {
            $productOnline = $rowData['product_online'] != 1 ? 2 : 1;
        }

        //If new price is below threshold, always disable
        if (isset($rowData['price'])) {
            $thresholdPrice = $this->helper->getThresholdPrice();
            if (isset($thresholdPrice) && $rowData['price'] <= $thresholdPrice) {
                $productOnline = 2;
            }
        }

        $rowData['product_online'] = $productOnline;
    ...
    public function validateData()
    {
        if (!$this->_dataValidated) {
            ...
            if ($this->getEntityTypeCode() == 'catalog_product') {
                $existingColnames = $this->getSource()->getColNames();
                $customColnames = $this->customColNames();
                $finalColumns = array_merge($existingColnames, $customColnames);
                $colnames = $finalColumns;
            }
     ...
     * @return $this
     */
    protected function _initAllWebsites()
    {
        $websites = $this->storeManager->getWebsites();
        $websitesList = [];
        foreach ($websites as $website) {            
            $websitesList[$website->getId()] = $website->getCode();
        }
        $this->_allWebsites = $websitesList;
        return $this;     
    }
     ...
     * @return array
     */
    public function getWebsiteIdsByCode(string $code): array
    {
        $websiteCodes = explode($this->getMultipleValueSeparator(), $code);
        $websites = array_flip($this->_allWebsites);
        $websitesIds = [];        
        foreach($websiteCodes as $websiteCode) {
            if(in_array($websiteCode, $websites)) {
                $websitesIds[$websiteCode] = $websites[$websiteCode];
            }
        }
        return $websitesIds;
    }
    ...
    /**
     * Get Existing product Gallery data
     * 
     * @param array $bunch
     * @return array
     */
    public function getExistingProductMediaGallery(array $bunch) : array
    {
        ...
        $productMediaGalleryeData = $this->_connection->fetchAll($select);

        foreach($productMediaGalleryeData as $productMedia){
            foreach($productSKUs as $productSKU){
                if($productSKU == $productMedia['sku']){
                    $productmediaData[$productMedia['sku']][] = $productMedia['value'];
                }
            }
        }	
