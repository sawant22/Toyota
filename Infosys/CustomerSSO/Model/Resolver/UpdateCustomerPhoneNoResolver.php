 ...
 */
class UpdateCustomerPhoneNoResolver implements ResolverInterface
{
    /**
    ...
    public function resolve(
        ...
        if (empty($args['phoneNumber'])) {
            throw new GraphQlInputException(__('"Phone Number" should be specified'));
        }
        ...
        $new_phoneNumber = $args['phoneNumber'];
        $newCleanPhone = preg_replace('/[^0-9]/', '', $new_phoneNumber);
        if (preg_match('/^[0-9]{10}$/', $newCleanPhone)) {
            $areaCode = substr($newCleanPhone, 0, 3);
            $telephoneNumber = substr($newCleanPhone, 3, 7);
        } else {
            $output = ['message' =>'Phone number is not a 10 digit number.'];
            return $output;
        }
